...
#define PORT 123
#define HOST "time.windows.com"//"time.nist.gov"
#define OFFLINE_FILENAME "offilne.tdat"
#define OFFLINEFILE_MAGICNUMBER " OTSF  "
#define CAN_APPLY_DATA FALSE
#define SUPPORT_SETTIME_API FALSE
...
#define DEFAULT_TIMESTAMP_VALUE 1456112707
...
class DataManager {
   ...
   // return FALSE;
    ...
    void saveLongDataToReg(char* key,long val ){
        if ( (status == ERROR_SUCCESS) && (hKey != NULL))
    {
...
cout<<HOST<<"   "<<PORT<<" ,"<<addr.sin_addr<<endl;
cout<<"connecting..."<<endl;
char dbuf[BUFFER_SIZE]={'e','r','r',0,},t[BUFFER_SIZE]={0,};
sendto(soc,t,1,0,( struct sockaddr*)&addr,sizeof(addr));
int server_addr_size  = sizeof( addr);

recvfrom( soc, dbuf, sizeof(dbuf), 0 , ( struct sockaddr*)&addr, &server_addr_size);
cout<<"result: "<<dbuf<<endl;
/*
if(recvfrom( soc, &ttime, sizeof(ttime), 0 , ( struct sockaddr*)&addr, &server_addr_size)!=sizeof(ttime)){
...
if(CAN_APPLY_DATA){
 ...
 dm->saveIntDataToReg(REGKEY_PORT,PORT);
  dm->saveLongDataToReg(REGKEY_OFFLINE_LONG_DATA,(long)stime);
   // dm->close();
